...
def test_poll_blocks_timeout(
    ...
    with (
        pytest.raises(ProviderError, match=r"Timed out waiting for next block."),
        PollDaemon("blocks", poller, lambda x: None, lambda: False),
    ):
        time.sleep(1.5)
...
def test_poll_blocks_reorg_with_timeout(
    ...
    with (
        pytest.raises(ProviderError, match=r"Timed out waiting for next block."),
        PollDaemon("blocks", poller, blocks.put, blocks.full),
    ):
        # Sleep first to ensure listening before mining.
        time.sleep(1)
