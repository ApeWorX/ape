...
contract SoladyFactory {
    event Target(address addr);

    function deploySoladyPush(address _implementation) external returns (address split){
        split = LibClone.clone_PUSH0(0, _implementation);
        emit Target(split);
    }

    function deploySoladyCWIA(address _implementation) external returns (address split){
