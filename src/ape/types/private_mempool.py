...
from eth_pydantic_types.hex import HexBytes, HexBytes32, HexInt, HexStr, HexStr32
...
class Validity(BaseModel):
    ...
    refund_config: Optional[list[RefundConfig]] = Field(default=None, alias="refundConfig")
    """
    Specifies what addresses should receive what percent of the overall refund for this bundle,
    if it is enveloped by another bundle (e.g. a searcher backrun).
    """
...
class EthBundle(BaseModel):
    """
    An ETH namespace bundle of transactions to send to the matchmaker.
    """
    ...
    min_timestamp: Optional[int] = Field(default=None, alias="minTimestamp")
    """
    The minimum timestamp for which this bundle is valid, in seconds since the unix epoch.
    """

    max_timestamp: Optional[int] = Field(default=None, alias="maxTimestamp")
    """
    The maximum timestamp for which this bundle is valid, in seconds since the unix epoch.
    """

    reverting_tx_hashes: list[HexStr32] = Field(default=None, alias="revertingTxHashes")
    """
    A list of tx hashes that are allowed to revert.
    """

    replacement_uuid: Optional[str] = Field(default=None, alias="replacementUuid")
    """
    ...
    builders: Optional[list[str]] = None
    """
    A list of `registered <https://github.com/flashbots/dowg/blob/main/builder-registrations.json>`__
    block builder names to share the bundle with.
    """


class EthCallBundle(BaseModel):
    txs: list[HexStr] = []
    """
    A list of signed transactions to execute in an atomic bundle.
    """
    ...
    state_block_number: HexInt | str = Field(default="latest", alias="stateBlockNumber")
    """
    Block number hex or int or tag for which state to base this simulation on.
    """
...
class EthSendBundleResult(BaseModel):
    bundle_hash: str = Field(alias="bundleHash")
    """
    The result from a bundle request.
    """

    smart: bool = False
    """
    ``True`` if using the smart bundle feature (when specified `builders` in the request).
    """


class Bundle(BaseModel):
    """
    An MEV namespace bundle of transactions to send to the matchmaker.
    """
...
class EthCallSimulateResult(BaseModel):
    """
    Transaction results from an ``eth_simBundle`` report.
    """

    coinbase_diff: str = Field(alias="coinbaseDiff")
    """
    The net balance change (in wei) for the block producer (miner or validator).
    """

    eth_sent_to_coinbase: str = Field(alias="ethSentToCoinbase")
    """
    The total ETH (in wei) sent directly to the coinbase address within the bundle.
    """

    from_address: AddressType = Field(alias="fromAddress")
    """
    The address that sent the transaction.
    """

    gas_fees: str = Field(alias="gasFees")
    """
    The total gas fees (in wei) generated by the transaction.
    """

    gas_price: str = Field(alias="gasPrice")
    """
    Gas price (in wei per unit of gas) specified for the transaction.
    """

    gas_used: int = Field(alias="gasUsed")
    """
    Total amount of gas consumed by executing this transaction.
    """

    to_address: AddressType = Field(alias="toAddress")
    """
    The recipient address of the transaction.
    """

    tx_hash: HexStr32 = Field(alias="txHash")
    """
    The unique transaction hash (32-byte hex string) identifying this transaction.
    """

    value: str = Field(alias="value")
    """
    Amount of ETH (in wei) transferred from the sender to the recipient.
    """


class EthCallSimulationReport(BaseModel):
    """
    A simulation result returned from the ``eth_callBundle`` RPC method.
    """
    ...
    bundle_gas_price: str = Field(alias="bundleGasPrice")
    """
    The effective gas price (in wei) for the entire bundle.
    """
    ...
    gas_fees: str = Field(alias="gasFees")
    """
    The total gas fees (in wei) generated by the bundle.
    """

    results: list[EthCallSimulateResult] = []
    """
    Results from each transaction in the bundle.
    """
