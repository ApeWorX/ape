...
from typing import TYPE_CHECKING, Any, Dict, Iterator, List, Optional, TypeVar, cast
...
class PluginConfig(BaseSettings):
    ...
    def __getattr__(self, attr_name: str) -> Any:
        ...
        if attr_name == "__isabstractmethod__":
            # Prevents useless logs in DEBUG output.
            return False
...
class ApeConfig(ExtraAttributesMixin, BaseSettings, ManagerAccessMixin):
    ...
    **NOTE**: Non-absolute paths are relative to the project-root.
    ...
    def __ape_extra_attributes__(self) -> Iterator[ExtraModelAttributes]:
        ...
        class PluginLoader(dict):
            def __contains__(self, item):
                return root.__contains__(item.replace("-", "_"))

            def __getattr__(self, item):
                return root.get_config(item.replace("-", "_"))
        ...
        yield ExtraModelAttributes(
            name="plugin_configs",
            attributes=lambda: PluginLoader(),
    ...
    def get_config(self, plugin_name: str) -> PluginConfig:
        ...
        return (
            self.get_plugin_config(name) or self.get_custom_ecosystem_config(name) or PluginConfig()
        )
    ...
    def get_plugin_config(self, name: str) -> Optional[PluginConfig]:
        ...
        for plugin_name, config_class in self.plugin_manager.config_class:
            ...
            if config_class != ConfigDict:
                ...
                config = config_class.from_overrides(cfg)
            else:
