...
class LogInputABICollection:
    def __init__(self, abi: EventABI):
        self.abi = abi
        self.topic_abis = [i for i in abi.inputs if i.indexed]
        self.data_abis: List[EventABIType] = [i for i in abi.inputs if not i.indexed]
    ...
    def decode(self, topics: List[str], data: str) -> Dict:
        decoded = {}
        for abi, topic_value in zip(self.topic_abis, topics[1:]):
        ...
        data_abi_types = [abi.canonical_type for abi in self.data_abis]
        hex_data = decode_hex(data) if isinstance(data, str) else data
        data_values = decode(data_abi_types, hex_data)

        for abi, value in zip(self.data_abis, data_values):
