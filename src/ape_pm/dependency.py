...
class LocalDependency(DependencyAPI):
    ...
    def fetch(self, destination: Path):
        ...
        if self.local.is_dir():
            project = self.Project(self.local)
...
class GithubDependency(DependencyAPI):
    ...
    **NOTE**: Will be ignored if given a 'ref'.
    ...
    def fetch(self, destination: Path):
        ...
        if self.ref:
            github_client.clone_repo(self.github, destination, branch=self.ref)

        else:
            try:
                github_client.download_package(self.github, self.version or "latest", destination)
                ...
                try:
                    github_client.clone_repo(self.github, destination, branch=self.version)
                except Exception:
...
class NpmDependency(DependencyAPI):
    ...
    def validate_local_npm(cls, model):
        ...
        if npm.is_absolute():
            ...
            # Note: Generally, they should be resolved at model
            #  construction time, if parsing a config file normally.
            project = Path(model.pop("project"))
            for base_path in (project, Path.home()):
        ...
        if not npm.is_dir():
            raise ProjectError(f"(model validation) NPM package '{npm}' not installed.")
